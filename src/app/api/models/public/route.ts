...
export async function GET(request: Request) {
  ...
  const response = await fetch(`${url}/rest/v1/rpc/get_public_model_leaderboard`, {
    ...
    body: JSON.stringify({
      requested_user: user.id, requested_sport: params.get("sport") || null,
  ...
  if (!response.ok) return Response.json({ error: "Public model rankings are temporarily unavailable." }, { status: 503 });
