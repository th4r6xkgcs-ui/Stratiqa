...
export async function POST(request: Request) {
  ...
  const externalTicket = form.get("claimType") === "external_ticket";
  ...
  if ((!externalTicket && !pickId) || !sportsbook || (!ticketId && !(image instanceof File))) return Response.json({ error: externalTicket ? "Add the sportsbook and a ticket ID or screenshot." : "Choose a locked pick and add a ticket ID or screenshot." }, { status: 400 });
  ...
  const pickResponse = externalTicket ? null : await fetch(`${url}/rest/v1/graded_betting_activity?id=eq.${encodeURIComponent(pickId)}&user_id=eq.${user.id}&source=eq.provider&select=id,sportsbook,selection,event_name,event_commence_at,certification_status&limit=1`, { headers: serviceHeaders, cache: "no-store" });
  ...
  if (!externalTicket && !pick) return Response.json({ error: "That locked pick could not be found." }, { status: 404 });
  if (pick?.certification_status === "certified") return Response.json({ error: "That pick is already STRATIQA Certified." }, { status: 409 });
  if (pick && pick.sportsbook.toLowerCase() !== sportsbook.toLowerCase()) return Response.json({ error: `Use the ticket from ${pick.sportsbook}, where this line was locked.` }, { status: 409 });
  if (pick && !pick.event_commence_at) return Response.json({ error: "This older pick has no verifiable start time and cannot become certified." }, { status: 409 });
  if (pick?.event_commence_at && new Date(pick.event_commence_at).getTime() <= Date.now()) return Response.json({ error: "Evidence for an existing STRATIQA pick must be submitted before the event starts." }, { status: 409 });
  ...
  if (image instanceof File) {
    ...
    try {
      ...
      if (extraction) {
        matchScore = pick ? scoreTicketMatch({ sportsbook: pick.sportsbook, selection: pick.selection, eventName: pick.event_name }, extraction) : null;
        extractionStatus = matchScore !== null && matchScore >= 75 ? "ready" : "review";
      }
    } catch (error) {
  ...
  const insert = await fetch(`${url}/rest/v1/pick_evidence`, {
    ...
    body: JSON.stringify({
      user_id: user.id, pick_id: pick?.id ?? null, sportsbook, ticket_id: ticketId ?? extraction?.ticketId ?? null,
      object_path: objectPath, content_hash: contentHash, verification_status: "pending", rating_eligible: false,
      ...(externalTicket ? { claim_type: "external_ticket" } : {}),
  ...
  const message = externalTicket
    ? "External ticket saved for independent review. It has no rating impact unless STRATIQA verifies the placement time, market, and official result."
