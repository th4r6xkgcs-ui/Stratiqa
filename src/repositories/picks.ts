...
export type NewPick = Omit<TrackedPick, "id" | "userId" | "closingOdds" | "result" | "profitUnits" | "source" | "verificationStatus" | "providerEventId" | "providerSportKey" | "marketKey" | "outcomeName" | "linePoint" | "attributionType" | "modelId" | "modelVersion" | "modelName" | "pickOrigin" | "placedAt" | "gradedAt">;
...
class SupabasePicksRepository implements PicksRepository {
  ...
  async createProviderBatch(userId: string, picks: ProviderPick[]) {
    ...
    const rows = picks.map((pick) => ({ user_id: userId, sport: pick.sport, category: pick.category, event_name: pick.eventName, selection: pick.selection, market: pick.market, sportsbook: pick.sportsbook, american_odds: pick.americanOdds, stake_units: pick.stakeUnits, confidence: pick.confidence, result: "pending", source: "provider", verification_status: "pending", provider_event_id: pick.providerEventId, provider_sport_key: pick.providerSportKey, market_key: pick.marketKey, outcome_name: pick.outcomeName, line_point: pick.linePoint, participant_name: pick.participantName ?? null, attribution_type: pick.attributionType, model_id: pick.modelId, model_version: pick.modelVersion, model_name: pick.modelName, pick_origin: pick.pickOrigin, pick_card_id: pick.pickCardId ?? cardId, locked_at: now, placed_at: now }));
