...
export function ModelWorkshop() {
  ...
  const loadMemory = useCallback(() => fetch("/api/models/history", { cache: "no-store" }).then((response) => response.ok ? response.json() : { history: [] }).then((result) => { setMemory(result.history ?? []); setCalibration(result.calibration ?? null); }).catch(() => undefined), []);
