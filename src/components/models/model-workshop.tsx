...
import { ArrowLeft, ArrowRight, BrainCircuit, Check, ChevronDown, EyeOff, Gauge, GitCompareArrows, Plus, Pencil, ShieldCheck, Sparkles, Target, Users, Zap } from "lucide-react";
...
export function ModelWorkshop() {
  ...
  const loadRecommendations = () => fetch("/api/models/recommendations", { cache: "no-store" }).then((response) => response.ok ? response.json() : { recommendations: [] }).then((result) => { setRecommendations(result.recommendations ?? []); setPasses(result.passes ?? []); setDesk(result.desk ?? null); }).catch(() => undefined);
