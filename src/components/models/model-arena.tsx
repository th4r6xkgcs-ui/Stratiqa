...
export function ModelArena({ models }: { models: ManagedModel[] }) {
  ...
  return <section className="model-arena">
    ...
    <div className="model-arena-summary">
      <Card><Crown /><span><small>YOUR TOP MODEL</small><strong>{best?.name ?? "Build your first model"}</strong><p>{best ? `${best.performance.rating} rating · ${best.sport} ${categoryLabel(best.category)}` : "Every model starts at 1500"}</p></span></Card>
    ...
    <Card className="model-arena-board">
      ...
      {loading ? <div className="model-arena-loading">{[1, 2, 3, 4].map((item) => <i key={item} />)}</div> : leaders.length ? leaders.map((leader) => <details key={leader.model_id} className={leader.is_current_user ? "owned" : ""}>
        <summary>
          <b>#{leader.rank}</b>
          <span><strong>{leader.model_name}{leader.is_current_user ? " (Yours)" : ""}</strong><small>{leader.owner_alias} · {leader.sport} · {categoryLabel(leader.category)} · v{leader.version}</small></span>
        ...
        <div><span><small>ACCURACY</small><strong>{leader.wins + leader.losses ? (leader.wins / (leader.wins + leader.losses) * 100).toFixed(1) : "0.0"}%</strong></span><span><small>MODEL ROI</small><strong>{Number(leader.roi_percent).toFixed(1)}%</strong></span><span><small>SAMPLE</small><strong>{leader.graded_picks >= 50 ? "Established" : "Developing"}</strong></span><span><small>PUSHES</small><strong>{leader.pushes}</strong></span></div>
      </details>) : <div className="model-arena-empty"><BrainCircuit /><strong>This arena is wide open</strong><p>Promote a model to your active lineup and settle 10 attributed picks to claim a ranking.</p></div>}
    </Card>
  </section>;
}
