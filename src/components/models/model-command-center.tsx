...
export function ModelCommandCenter({ models, onModels, onStatus }: { models: ManagedModel[]; onModels: (models: ManagedModel[]) => void; onStatus: (status: string) => void }) {
  ...
  return <section className="model-command-center">
    ...
    {history ? <div className="model-editor-backdrop"><Card className="model-editor model-history"><header><div><span className="landing-kicker">IMMUTABLE HISTORY</span><h2>{history[0]?.name} versions</h2></div><button onClick={() => setHistory(null)}><X /></button></header><p>Every verified pick keeps the version that produced it. Older versions cannot be edited.</p><div>{history.map((version) => <article key={version.version}><span><b>v{version.version}</b>{version.current ? <Badge tone="success">CURRENT</Badge> : <Badge>ARCHIVED</Badge>}</span><strong>{version.name}</strong><small>{factorLabel(version.strategy)} first · {version.factors.length} signals · {version.risk_profile}</small></article>)}</div><footer><Button onClick={() => setHistory(null)}>Done</Button></footer></Card></div> : null}
  </section>;
}
