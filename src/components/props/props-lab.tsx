...
export function PropsLab({ props, provider, updatedAt }: { props: PropData[]; provider: string; updatedAt: string }) {
  ...
  const savedProps = useMemo(() => props.filter((prop) => favorites.includes(prop.id)), [favorites, props]);
  ...
  return <div>
    ...
    {showSaved && savedProps.length > 1 ? <section className="props-comparison"><header><div><strong>Compare saved props</strong><small>Signals are for research—not a guarantee. Choose each line deliberately.</small></div><button type="button" aria-expanded={showComparison} onClick={() => setShowComparison((value) => !value)}>{showComparison ? "Hide comparison" : `Compare ${savedProps.length} props`}</button></header>{showComparison ? <div className="props-comparison-table"><div className="props-comparison-head"><span>Player · market</span><span>Line</span><span>Hit rate</span><span>Projection</span><span>EV</span><span>Confidence</span><span /></div>{savedProps.map((prop) => <article key={prop.id}><span><strong>{prop.player}</strong><small>{prop.market} · {prop.matchup}</small></span><b>{prop.line}</b><b>{prop.hitRate}%</b><b>{prop.projection}</b><b className="positive">+{prop.expectedValue}%</b><b>{prop.confidence}%</b><button type="button" aria-label={`Add ${prop.player} to slip`} onClick={() => addProp(prop)}><Plus /> Add</button></article>)}</div> : null}</section> : null}
