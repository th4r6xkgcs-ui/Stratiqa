...
import { Bookmark, ChevronDown, Plus, Search, ShieldCheck, X } from "lucide-react";
...
export function PropsLab({ props, provider, updatedAt }: { props: PropData[]; provider: string; updatedAt: string }) {
  ...
  const [selectedProp, setSelectedProp] = useState<PropData | null>(null);
  ...
  return <div>
    ...
    {visible.length ? <section className="props-grid">{visible.map((prop) => <Card className="prop-card glass-card" key={prop.id}>
      ...
      <button type="button" className="prop-player" onClick={() => setSelectedProp(prop)} aria-label={`Open research for ${prop.player}`}><PlayerAvatar name={prop.player} sportKey={prop.providerSportKey} /><div><h2>{prop.player}</h2><p>{prop.market} · {prop.point ?? prop.line.replace(/^(Over|Under) /, "")}</p></div><small>{prop.team || leagueNames[prop.providerSportKey ?? ""] || "PROP"}</small></button>
      ...
      <details className="prop-deep-research"><summary>Open player research <ChevronDown /></summary><div className="prop-metrics"><span><small>Hit rate</small><strong>{prop.hitRate}%</strong></span><span><small>Expected value</small><strong>+{prop.expectedValue}%</strong></span><span><small>Confidence</small><strong>{prop.confidence}%</strong></span></div><div className="prop-chart-row"><span><small>Last 7</small><TrendBars values={prop.trend} /></span><b>Projection {prop.projection}</b></div><div className="prop-research-note"><p><strong>Hit rate</strong> summarizes the recent tracked sample. <strong>Projection</strong> is the current provider or model estimate. Expected value and confidence are analysis signals, not guaranteed outcomes.</p><span><ShieldCheck /> {prop.live ? "This provider line can be locked before start and settled automatically." : "This fallback analysis is educational only and cannot be locked for ratings."}</span></div><footer><div>{prop.tags.slice(1).map((tag) => <span key={tag}>{tag}</span>)}</div><small>{prop.live ? "LIVE PROVIDER MARKET" : "DEMO ANALYSIS"}</small></footer></details>
    ...
    {selectedProp ? <div className="player-research-overlay" role="dialog" aria-modal="true" aria-label={`${selectedProp.player} research`}><button className="player-research-backdrop" aria-label="Close player research" onClick={() => setSelectedProp(null)} /><aside className="player-research-drawer"><header><span>PLAYER RESEARCH</span><button type="button" aria-label="Close player research" onClick={() => setSelectedProp(null)}><X /></button></header><div className="player-research-hero"><PlayerAvatar name={selectedProp.player} sportKey={selectedProp.providerSportKey} /><div><small>{selectedProp.team || leagueNames[selectedProp.providerSportKey ?? ""] || "PROP"} · {selectedProp.matchup}</small><h2>{selectedProp.player}</h2><p>{selectedProp.market} · {selectedProp.point ?? selectedProp.line}</p></div></div><section className="player-research-score"><span><small>Research score</small><strong>{researchPriority(selectedProp)}<em>/100</em></strong></span><p>Ranks current model confidence, recent hit rate, and expected value. It is research—not a guarantee.</p></section><section className="player-research-grid"><span><small>Hit rate</small><strong>{selectedProp.hitRate}%</strong></span><span><small>Projection</small><strong>{selectedProp.projection}</strong></span><span><small>EV</small><strong>+{selectedProp.expectedValue}%</strong></span><span><small>Confidence</small><strong>{selectedProp.confidence}%</strong></span></section><section className="player-research-trend"><header><span>Recent tracked trend</span><small>Last 7</small></header><TrendBars values={selectedProp.trend} /></section><section className="player-research-context"><strong>Before you lock</strong><p>Confirm the line, expected role, injury news, and game start time. STRATIQA tracks locked pregame picks for ratings; this research panel does not create a pick by itself.</p></section><button type="button" className="player-research-add" onClick={() => { addProp(selectedProp); setSelectedProp(null); }}><Plus /> Add {selectedProp.line} to slip <b>{selectedProp.price > 0 ? "+" : ""}{selectedProp.price}</b></button></aside></div> : null}
  </div>;
}
