    ...
    {loading ? <section className="board-loading">{[1, 2, 3].map((item) => <i key={item} />)}</section> :
      visible.length ? <section className="sportsbook-board">{visible.map((event) => {
        ...
        return <Card className="sportsbook-event" key={event.id}>
          ...
          <div className="board-teams"><Link className="board-team-link" href={`/matchups/${event.slug}`} aria-label={`Open full research for ${event.awayTeam} at ${event.homeTeam}`}><i>{shortTeam(event.awayTeam)}</i><strong>{event.awayTeam}</strong><small>Away · Open research</small></Link><Link className="board-game-link" href={`/matchups/${event.slug}`} aria-label={`Open full research for ${event.awayTeam} at ${event.homeTeam}`}>GAME<br />RESEARCH</Link><Link className="board-team-link home" href={`/matchups/${event.slug}`} aria-label={`Open full research for ${event.awayTeam} at ${event.homeTeam}`}><i>{shortTeam(event.homeTeam)}</i><strong>{event.homeTeam}</strong><small>Home · Open research</small></Link></div>
          ...
          <details className="board-explore"><summary><span><Sparkles /> Explore this matchup</span><small>Stats, props, live tracking &amp; Coach</small><ChevronDown /></summary><nav aria-label={`More ways to research ${event.awayTeam} at ${event.homeTeam}`}><Link href={`/matchups/${event.slug}`}><Sparkles /> Full game report</Link><Link href="/props"><Target /> Player props</Link><Link href="/games"><CalendarDays /> Game Center</Link><Link href="/coach"><ShieldCheck /> AI Coach</Link></nav></details>
        </Card>;
      })}</section> :
