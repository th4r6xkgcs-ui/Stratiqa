...
export function PickSlip() {
  ...
  const analysis = useMemo(() => {
    ...
    return { confidence, ev, risk, grade, winGain, loss, correlated };
  }, [legs]);
  async function lock() {
    ...
    setSaving(true);
    const response = await fetch("/api/picks/verified/batch", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ units, legs: legs.map((leg) => ({ kind: leg.kind ?? "matchup", slug: leg.slug, propId: leg.propId, outcomeName: leg.outcomeName, book: leg.book, line: leg.selection, modelName: leg.modelName })) }) });
  ...
  return <>
    ...
    <aside className={open ? "global-slip open" : "global-slip"}>
      ...
      {legs.length ? <>
        <div className="global-slip-legs">{legs.map((leg) => <article key={leg.id}><button onClick={() => remove(leg.id)}><X /></button><small>{leg.eventName}</small><strong>{leg.selection}</strong><p>{leg.book}<b>{leg.price > 0 ? "+" : ""}{leg.price}</b></p>{!leg.live ? <em>DEMO · NOT RATING ELIGIBLE</em> : <em className="verified"><ShieldCheck /> LIVE LINE</em>}</article>)}</div>
        ...
        <label className="slip-units">Unit size<input type="number" min=".25" max="10" step=".25" value={units} onChange={(event) => setUnits(Number(event.target.value))} /></label>
        ...
        {analysis.correlated ? <p className="slip-note"><AlertTriangle /> This card contains selections from the same event. Confidence is reduced until correlation is fully modeled.</p> : null}
