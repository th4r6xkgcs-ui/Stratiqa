...
export function PickSlip() {
  ...
  const analysis = useMemo(() => {
    const confidence = legs.length ? Math.round(legs.reduce((product, leg) => product * leg.confidence / 100, 1) ** (1 / legs.length) * 100) : 0;
    ...
    return { confidence, ev, risk, grade, winGain, loss };
  }, [legs]);
  async function lock() {
    ...
    setSaving(true);
    const response = await fetch("/api/picks/verified/batch", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ units, legs: legs.map((leg) => ({ slug: leg.slug, book: leg.book, line: leg.selection, modelName: leg.modelName })) }) });
