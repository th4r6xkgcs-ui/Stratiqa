...
export function CertifiedLeaderboard() {
  ...
  if (profile === null) return <Card className="premium-empty"><Trophy /><strong>Loading certified rankings…</strong></Card>;
  return <div className="certified-rankings"><Card className="ranking-profile"><header><span><MapPin /> YOUR COMPETITIVE REGION</span><Badge tone={profile.leaderboard_opt_in ? "success" : "warning"}>{profile.leaderboard_opt_in ? "VISIBLE" : "PRIVATE"}</Badge></header><form onSubmit={save}><label>Public ranking name<input name="publicAlias" defaultValue={profile.public_alias ?? ""} maxLength={30} placeholder="Your analyst name" /></label><div><label>Country code<input name="countryCode" defaultValue={profile.country_code ?? ""} maxLength={2} placeholder="US" /></label><label>State / region<input name="regionCode" defaultValue={profile.region_code ?? ""} maxLength={12} placeholder="CA" /></label></div><label>City or metro <small>Optional—never enter an address</small><input name="locality" defaultValue={profile.locality ?? ""} maxLength={60} placeholder="Los Angeles" /></label><label className="ranking-optin"><input name="leaderboardOptIn" type="checkbox" defaultChecked={profile.leaderboard_opt_in} /><span><ShieldCheck /> Show my alias and certified stats after I reach 25 certified picks in a category.</span></label><Button><Check /> Save ranking profile</Button></form>{status ? <p>{status}</p> : null}</Card><section className="ranking-board"><header><div className="filter-tabs">{categories.map(([value, label]) => <button className={category === value ? "active" : ""} onClick={() => setCategory(value)} key={value}>{label}</button>)}</div><div className="filter-tabs">{(["global", "country", "region", "local"] as const).map((value) => <button className={scope === value ? "active" : ""} disabled={value !== "global" && !profile.country_code} onClick={() => setScope(value)} key={value}>{value}</button>)}</div></header><Card className="certified-table"><header><span>Rank</span><span>Analyst</span><span>Rating</span><span>Certified record</span></header>{leaders.length ? leaders.map((leader) => <article key={`${leader.category}-${leader.rank}-${leader.public_alias}`}><b>#{leader.rank}</b><span><strong>{leader.public_alias}</strong><small>{[leader.locality, leader.region_code, leader.country_code].filter(Boolean).join(", ")}</small></span><strong>{Math.round(leader.rating)}</strong><span>{leader.wins}-{leader.losses}<small>{leader.graded_picks} certified</small></span></article>) : <div className="ranking-empty"><Trophy /><strong>The top 10 is open</strong><p>Be among the first analysts to complete 25 certified {categories.find(([value]) => value === category)?.[1].toLowerCase()} picks in this region.</p></div>}</Card></section></div>;
}
