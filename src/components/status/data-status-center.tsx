    ...
    {health ? <>
      ...
      <Card className="provider-status-board">
        ...
        {health.providers.map((provider) => {
          const state = displayState(provider);
      ...
      <Card className="quota-policy">
        <header><ShieldCheck /> Free-first reliability policy</header>
        <div><span><strong>{health.policy.oddsCacheSeconds}s</strong><small>Game-odds cache</small></span><span><strong>{health.policy.propsCacheSeconds}s</strong><small>Props cache</small></span><span><strong>{health.policy.staleFallbackMinutes}m</strong><small>Maximum stale fallback</small></span><span><strong>$0 required</strong><small>Paid provider dependency</small></span></div>
      </Card>
    </> : null}
